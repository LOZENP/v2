...
function VM.generateRuntime(encryptedBlob, keyString, config)
    ...
    -- unpack compat (Lua5.1 uses unpack, Lua5.2+ uses table.unpack)
    w("local "..N[50].."=table.unpack or unpack")
    ...
    w("elseif op=="..OP.CALL.." then local fn="..N[21].."[A] local args={} for i=1,B-1 do args[i]="..N[21].."[A+i] end local rets={fn("..N[50].."(args))} for i=1,C-1 do "..N[21].."[A+i-1]=rets[i] end")
    w("elseif op=="..OP.TAILCALL.." then local fn="..N[21].."[A] local args={} for i=1,B-1 do args[i]="..N[21].."[A+i] end return fn("..N[50].."(args))")
    w("elseif op=="..OP.RETURN.." then local rets={} for i=0,B-2 do rets[i+1]="..N[21].."[A+i] end return "..N[50].."(rets)")
